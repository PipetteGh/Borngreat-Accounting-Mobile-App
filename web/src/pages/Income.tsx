...
import { 
  Plus, Search, ArrowDownCircle, 
  Calendar, MoreHorizontal, Trash2, Edit,
  X, Filter, ChevronLeft, ChevronRight, Activity, TrendingUp
} from 'lucide-react';
import { 
  DropdownMenu,
  DropdownMenuContent,
...
export default function Income() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [txRes, breakdownRes, accRes] = await Promise.all([
        ...
        api.get('/dashboard/category-breakdown?period=monthly&type=income').catch(() => ({ data: { breakdown: [] } })),
        accountService.getAccounts()
      ]);

      setIncomeTransactions(txRes.data.transactions || []);
      setBreakdown(breakdownRes.data.breakdown || []);
      setAccounts(accRes);
    } catch (error) {
      toast.error('Failed to sync income data');
    } finally {
      setLoading(false);
    }
  }, [user, dateRange]);
  ...
  useEffect(() => {
    fetchData();
    document.title = 'Income and Inflows | Borngreat Accounting';
  }, [fetchData]);
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 font-outfit">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-primary tracking-tighter uppercase">Income Inflows</h1>
          <p className="text-[#8E92A8] text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Prime Financial Inflow Monitoring</p>
        </div>
        <Button 
          onClick={() => { setEditingItem(null); setIsModalOpen(true); }}
          className="bg-accentGreen hover:bg-accentGreen/90 text-white rounded-lg h-9 px-5 gap-1.5 font-black text-[9px] uppercase tracking-widest shadow-sm"
        >
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card className="rounded-xl border-border bg-white shadow-sm overflow-hidden">
            <CardHeader className="p-6 pb-2">
               <CardTitle className="text-sm font-black uppercase tracking-widest text-secondary/60">Income Breakdown</CardTitle>
            </CardHeader>
          ...
          {/* Quick List - Simplified Recent Income */}
          <div className="space-y-2">
             {incomeTransactions.slice(0, 4).map((tx) => (
                <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl bg-white border border-border/50 shadow-sm group hover:border-accentGreen/50 transition-colors">
                   <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-accentGreen/10 flex items-center justify-center text-accentGreen">
                         <Plus className="w-4 h-4" />
                      </div>
                      <div>
                         <p className="text-[11px] font-black text-primary leading-tight truncate max-w-[140px]">{tx.description || tx.category_name}</p>
                         <p className="text-[8px] font-black text-secondary/40 uppercase tracking-widest leading-none mt-0.5">Inflow</p>
                      </div>
                   </div>
                   <p className="text-xs font-black text-accentGreen">+{currency}{parseFloat(tx.amount).toLocaleString()}</p>
                </div>
             ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <Card className="rounded-xl border-border bg-white shadow-sm min-h-[450px] flex flex-col">
            <CardHeader className="p-6 pb-2 flex flex-row items-center justify-between">
               <div>
                  <CardTitle className="text-sm font-black uppercase tracking-widest text-secondary/60">Inflow Statements</CardTitle>
               </div>
               <div className="flex items-center gap-2">
               ...
               <div className="flex flex-col md:flex-row gap-3 mb-6">
                  <div className="relative flex-1">
                     ...
                     <Input 
                        ...
                        className="h-11 rounded-xl bg-slate-50 border-none font-bold text-xs pl-12 focus-visible:ring-accentGreen"
                        value={searchTerm}
               ...
               <div className="space-y-0.5">
                  {loading ? (
                     <div className="py-20 flex flex-col items-center justify-center">
                        <Activity className="w-8 h-8 text-accentGreen/20 animate-spin mb-4" />
                     ...
                     paginatedTransactions.map((tx) => (
                        <div key={tx.id} className="grid grid-cols-5 items-center px-4 py-3 rounded-lg hover:bg-slate-50 transition-colors group">
                           ...
                           <div className="col-span-1">
                              <p className="text-[11px] font-bold text-primary truncate max-w-[120px]">{tx.description || 'Prime Inflow'}</p>
                           </div>
                           ...
                           <div className="col-span-1 flex justify-center">
                              <span className="text-[8px] font-black text-accentGreen/60 bg-accentGreen/5 px-2 py-0.5 rounded-lg border border-accentGreen/10 uppercase tracking-widest">
                                 Cleared
                              </span>
                           </div>
                           <div className="col-span-1 flex items-center justify-end gap-2">
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                 <Button 
                                    variant="ghost" 
                                    size="icon" 
                                    className="h-7 w-7 rounded-md hover:bg-accentGreen/5 text-secondary hover:text-accentGreen"
                              ...
                              <span className="text-sm font-black text-accentGreen tabular-nums tracking-tighter min-w-[80px] text-right">
                     ...
                     <div className="py-20 text-center space-y-4">
                        <TrendingUp className="w-10 h-10 text-secondary/5 mx-auto" />
         ...
         <DialogContent className="rounded-xl bg-white border-none max-w-[400px] p-0 overflow-hidden shadow-2xl">
            <DialogHeader className="p-6 pb-4 border-b border-border/50">
               <DialogTitle className="text-xl font-black text-primary tracking-tighter uppercase text-center">
                  {editingItem ? 'Edit Entry' : 'New Inflow'}
               </DialogTitle>
            </DialogHeader>
               ...
               <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div className="space-y-1.5">
                        <Label className="text-[9px] font-black uppercase text-secondary/60 tracking-widest pl-1">Inflow Magnitude ({currency})</Label>
                        <Input 
                            type="number"
                            step="0.01"
                            autoFocus
                            placeholder="0.00"
                            className="h-11 rounded-xl bg-slate-50 border-border/50 text-lg font-black px-4 focus:ring-accentGreen tracking-tighter"
                            value={formData.amount}
                            onChange={(e) => setFormData({...formData, amount: e.target.value})}
                        />
                     </div>
                     <div className="space-y-1.5">
                        <Label className="text-[9px] font-black uppercase text-secondary/60 tracking-widest pl-1">Inflow Classification</Label>
                        <Select onValueChange={(val) => setFormData({...formData, category_id: val})} value={formData.category_id}>
                           ...
                           <SelectContent className="z-[300] bg-white border border-border rounded-xl">
                              ...
                              <div className="overflow-y-auto max-h-[180px]">
                                 {INCOME_CATEGORIES.filter(c => c.name.toLowerCase().includes(categorySearch.toLowerCase())).map((c) => (
                  ...
                  <div className="space-y-1.5">
                     ...
                     <Textarea 
                        ...
                        className="rounded-xl h-20 bg-slate-50 border-border/50 font-bold text-xs p-3 focus:ring-accentGreen"
            ...
            <DialogFooter className="p-6 pt-3 border-t border-border/50 bg-slate-50 flex-row gap-3">
               ...
               <Button onClick={handleSubmit} className="flex-1 bg-primary text-white h-10 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-md hover:scale-105 active:scale-95 transition-all">
                  {editingItem ? 'Update' : 'Commit Inflow'}
               </Button>
            </DialogFooter>
         </DialogContent>
      </Dialog>
